...
- 초기화권 총 개수와 만료일별 개수 표시: 만료 빠른 순 정렬, 동일 만료일 묶기, 클릭하여 전체 일정 확인
- 만료 없음·만료일 조회 불가·일부 권한의 상세 정보 누락을 구분하며 사용 완료 권한은 제외

- 왼쪽 사이드바 업데이트 버튼: GitHub 새 버전 확인, 다운로드 진행 표시, SHA-256 검증과 설치 후 재시작
- 업데이트 중 작업·Goal 보호 및 기존 계정·에이전트 설정·작업 기록 보존

2.1.0 이전 설치에는 이번 설치파일을 한 번 덮어 설치하면 업데이트 버튼이 생깁니다. 삭제할 필요가 없으며, 이후 버전부터 앱 안에서 업데이트할 수 있습니다. Linux DEB 설치는 PolicyKit을 통한 관리자 승인이 필요하며, 앱 폴더 교체 권한이 없으면 설치파일 열기를 이용합니다.
