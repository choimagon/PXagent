...
function update() {
  ...
  updateAgents(); updateMain(); updateOffice(); updateClock(); updateMailbox(); updateInbox(); updateKeyboardSound(); updateAssignmentButtons();updateOfficeTabs();
...
function updateAgents() {
  ...
  if (list.contains(document.activeElement) && document.activeElement.tagName==='SELECT') {
    state.agents.forEach(a=>{
      ...
      status.textContent = agentStatus(a); status.classList.toggle('working',a.status==='running');
    ...
    const expanded = selected ? `
      <div class="agent-expanded">
        ...
        <div class="agent-actions">
          <button class="small-button" data-action="${a.id==='secretary'?'secretary':'assign'}" data-id="${a.id}" ${a.status==='running'?'disabled':''}>${icon('plus',13)}${a.id==='secretary'?'진행 상황 묻기':a.id==='autoresearch'?'실험 요청':'작업 배정'}</button>
    ...
    return `${section}
      <article class="agent-row ${selected?'selected':''} ${a.reportsTo==='dev'?'junior-agent':''}" data-row="${a.id}">
        <button class="agent-select" data-select-agent="${a.id}" aria-expanded="${selected}">
          ...
          <span class="agent-state ${a.status==='running'?'working':''}">${agentStatus(a)}</span>${icon('down',12)}
        </button>
        ${expanded}
      </article>`;
  }).join('');
...
const phaseNames={thinking:'생각 중',coding:'구현 중',testing:'검증 중',reviewing:'검수 중',analyzing:'문서 분석 중',experimenting:'실험 중',started:'작업 준비',assigned:'작업 준비'};
const agentStatus=a=>a.status==='running'?(phaseNames[a.phase]||'실행 중'):'대기 중';
...
function updateOffice() {
  state.agents.forEach(a=>{
    ...
    el.dataset.phase=a.phase||'coding';const bubble=el.querySelector('.work-bubble text');if(bubble){bubble.textContent=({testing:'✓',reviewing:'?',analyzing:'≡',experimenting:'⚗'})[a.phase]||'···';bubble.setAttribute('font-size',a.phase==='coding'?'18':'13');}
    ...
    el.setAttribute('aria-label',`${a.name} · ${agentStatus(a)} · ${profiles[a.profile]} · 추론 ${REASONING_LEVELS[a.reasoningEffort]} · 설정 열기`);
...
function newTask(agentId='chief', sample='', parentTaskId='') {
  if(agentId==='autoresearch'){agentId='dev';sample ||= '반복 실험이 적합한지 코드를 확인하고 AutoResearch로 개선해줘. metric과 검증 명령을 먼저 확인해줘.';}
  ...
  openModal(`<h2>무슨 일을 맡길까요?</h2>
    <form id="task-form"><label class="field-label" for="task-description">작업 내용</label><textarea id="task-description" name="description" rows="5" required maxlength="12000" placeholder="할 일과 작업할 폴더를 적어주세요">${esc(sample)}</textarea>
      ...
      <details class="execution-options"><summary>검증 · 실험 제한</summary><label class="field-label">검증 명령 (한 줄에 하나)</label><textarea name="validationCommands" rows="3" placeholder="npm test&#10;npm run build">${esc((parent?.validationCommands||[]).join('\n'))}</textarea><p class="field-hint">비워두면 프로젝트의 lint·test·build 스크립트를 실행해요. AutoResearch는 개발 팀장이 적합성을 판단하며, 실제 metric과 테스트가 있어야 실행해요.</p><div class="research-limits">${[['maxIterations','최대 실험 회차',5],['maxMinutes','최대 시간 (분)',10],['maxTokens','토큰 예산',20000],['maxFiles','최대 변경 파일',10]].map(([key,label,max])=>`<label>${label}<input type="number" name="${key}" min="1" max="${max}" value="${parent?.researchLimits?.[key]||max}"></label>`).join('')}</div><p class="field-hint">토큰은 각 Codex 턴의 실제 사용량을 확인해 다음 회차를 차단해요. 진행 중인 턴은 예산을 넘길 수 있어요.</p></details>
...
function presetSummary(values) {
  return `<div class="preset-summary">${state.agents.map(a=>{const value=FIXED_AGENT_IDS.includes(a.id)?{profile:'luna',reasoningEffort:'medium'}:(values[a.id]||a);return `<div><span>${esc(a.name)}</span><b>${profiles[value.profile]} / ${value.reasoningEffort[0].toUpperCase()+value.reasoningEffort.slice(1)}</b></div>`;}).join('')}</div>`;
}
function agentPresets() {
  const current=agentPresetValues(state.agents);
  const matches=values=>state.agents.every(a=>{const value=FIXED_AGENT_IDS.includes(a.id)?{profile:'luna',reasoningEffort:'medium'}:(values[a.id]||a);return value&&a.profile===value.profile&&a.reasoningEffort===value.reasoningEffort;});
...
function settings() {
  openModal(`<div class="modal-eyebrow">MAKE YOURSELF AT HOME</div><h2>사무실 설정</h2><p class="modal-intro">PC에 로그인된 Codex로 동료들이 실제 작업을 수행해요.</p>${window.pxDesktop?'<button class="button subtle-button" data-action="desktop-setup">Codex 계정·연결 설정</button>':''}
    <form id="settings-form">
      ...
      <div class="form-notice">지정한 실제 폴더에서 직접 작업해요. 작업 폴더 밖의 파일도 요청에 필요하면 읽고 수정할 수 있어요. Codex는 기존 ChatGPT 로그인을 사용해요. 모델·추론 레벨·고정 프롬프트는 에이전트 상세 설정에서 바꿀 수 있어요.</div>
...
modal.addEventListener('submit',async event=>{
  ...
  try {
    ...
    if(form.id==='task-form') { data.validationCommands=String(data.validationCommands||'').split('\n').map(line=>line.trim()).filter(Boolean);data.researchLimits={};for(const key of ['maxIterations','maxMinutes','maxTokens','maxFiles']){data.researchLimits[key]=Number(data[key]);delete data[key];}const isGoal=form.querySelector('[data-action="toggle-goal"]').getAttribute('aria-pressed')==='true'; await api(isGoal?'goals':'tasks','POST',{...data,tailWeb:form.querySelector('[data-action="toggle-tail-web"]').getAttribute('aria-pressed')==='true',...(!isGoal?{requireIdle:true}:{})}); modal.close(); if(isGoal) { view='tasks'; renderView(); } toast(isGoal?'Goal을 맡겼어요. 달성까지 이어서 진행해요.':'작업을 맡겼어요. 보고는 편지함으로 도착해요.'); }
