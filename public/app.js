...
async function flushAgentSave(id,scope=selectedComputerId) {
  ...
  entry.inflight = api(`agents/${id}`, 'PATCH', {...patch,officeId:scope}).then(result => {
    ...
    if(scope===selectedComputerId){updateAgents();updateOffice();}
  }).catch(error => {
    ...
    return `${section}
      <article class="agent-row ${selected?'selected':''} ${a.reportsTo==='dev'?'junior-agent':''}" data-row="${a.id}">
        <button class="agent-select" data-select-agent="${a.id}" aria-expanded="${selected}">
          ...
          <span class="agent-info"><b>${a.name}${a.id==='chief'?'<span class="chief-tag">LEAD</span>':''}</b><small>${a.role}</small>${modelSummary(a)}</span>
...
function secretaryDetails() {
  ...
  openModal(`<div class="modal-agent-heading"><span class="avatar">${avatar('secretary',52)}</span><div><div class="modal-eyebrow">사장실 비서</div><h2>비둘기</h2><p>사장님, 지금 무슨 일이 진행 중인지 알려드릴게요.</p></div></div><div class="secretary-model-settings"><label class="field-label" for="secretary-profile">모델</label><select id="secretary-profile">${options(a.profile)}</select><label class="field-label" for="secretary-reasoning">추론 레벨</label><select id="secretary-reasoning">${reasoningOptions(a.profile,a.reasoningEffort)}</select><p class="field-hint">사장님 전용 비서 · 호문클루스가 배정하거나 제어할 수 없어요.</p><span id="secretary-save-status" role="status">변경하면 자동 저장돼요</span></div><div id="secretary-live" class="secretary-live"></div><div class="secretary-questions"><button class="button subtle-button" data-action="ask-secretary" data-question="호문클루스 지금 무슨 일 하고 있어?">무슨 일 하고 있어?</button><button class="button subtle-button" data-action="ask-secretary" data-question="현재 작업 진행 상황 어때?">진행 상황 어때?</button></div><form id="secretary-form"><label class="field-label" for="secretary-question">비둘기에게 물어보기</label><input id="secretary-question" name="question" required maxlength="2000" placeholder="호문클루스 작업 진행 상황 어때?"><button class="button primary" type="submit">물어보기</button></form><pre id="secretary-answer" class="result-text" aria-live="polite"></pre>`, 'secretary-modal');
  updateSecretary();
}
async function askSecretary(question) {
  ...
  answer.textContent='비둘기가 답변을 준비하고 있어요 · 결과 로딩 중…';
  try {
    while(saveEntry('secretary',scope)?.pending||saveEntry('secretary',scope)?.inflight) await flushAgentSave('secretary',scope);
    if(saveEntry('secretary',scope)?.failed)throw new Error('비둘기 설정을 저장하지 못했습니다. 모델을 다시 선택해주세요.');
...
modal.addEventListener('input',event=>{const form=event.target.closest('#agent-form');if(form) queueAgentSave(form);});
