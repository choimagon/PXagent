...
function updateKeyboardSound() {
  if (muted || document.hidden || view!=='office' || !connected || !state?.agents.some(agent=>agent.status==='running'&&(!agent.phase||agent.phase==='coding'))) {stopKeyboardSound();return;}
...
function settings() {
  openModal(`<div class="modal-eyebrow">MAKE YOURSELF AT HOME</div><h2>사무실 설정</h2><p class="modal-intro">PC에 로그인된 Codex로 동료들이 실제 작업을 수행해요.</p>${window.pxDesktop?'<button class="button subtle-button" data-action="desktop-setup">Codex 계정·연결 설정</button>':''}
    <form id="settings-form">
      ...
      <div class="form-notice">구현은 격리된 프로젝트 폴더에서 수행하고, 기획·분석·검수는 읽기 전용으로 실행해요. 실제 Validator 검사와 검수를 거쳐 원본에 반영해요. Codex는 기존 ChatGPT 로그인을 사용해요. 모델·추론 레벨·고정 프롬프트는 에이전트 상세 설정에서 바꿀 수 있어요.</div>
